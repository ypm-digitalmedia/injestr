...
$type = $_REQUEST['type'];
$wasabiUploadType = $_REQUEST['wasabiUploadType'];
//$sessionFolderName = $_SESSION['folderName'];

$doUpload = true;

if( $wasabiUploadType == "batch" && $type == "record" ) {
	$doUpload = false;
}
...
if (!empty($_FILES)) {
	...
	//		$targetChunkFile = $targetPath . $_FILES['file']['name'] . $ds . "[" . $_REQUEST['dzchunkindex'] . "-" . $_REQUEST['dztotalchunkcount'] . "]" . $_FILES['file']['name'];
			...
			if ( move_uploaded_file ($tempFile,$targetChunkFile)  ) {
				...
				switch ($_FILES['file']['error']) {  
					case 1:
	//					echo 'ERROR: The file is bigger than is allowed: ' . $fileName . ' ';
	...
	// END BLOCK - manual upload prevention
